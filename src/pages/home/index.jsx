...
const Home = () => {
  ...
  const menu = (
    <Menu>
      ...
      <Menu.Item key="Maison" onClick={(e) => setType("Maison")}>
        <div value="Maison">Maison</div>
      </Menu.Item>
      <Menu.Item key="reset" onClick={(e) => setType("")}>
        <div value="reset">reinitialiser</div>
      </Menu.Item>
    </Menu>
  );
  ...
  const picHouses = [
    "https://images.unsplash.com/photo-1567428485548-c499e4931c10?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=1500&q=80",
  ...
  const search = (e) => {
    ...
    if ((city !== "") & (type !== "")) {
      data.map(
        (annoucement) =>
          annoucement.price > inputValueMin &&
          annoucement.price < inputValueMax &&
          annoucement.size > sizeMin &&
          annoucement.size < sizeMax &&
          annoucement.city === city &&
          annoucement.typeHome === type &&
      ...
      <div className="FormFilter">
        <form className="Invisible">
          ...
          <div className="someOptionsForSearch">
            <div className="someOptionInput2">
              <Dropdown overlay={menu}>
                ...
                >
                  {type === "" ? "Que voulez vous acheter ? " : type}{" "}
                  <DownOutlined />
                </a>
              </Dropdown>
            </div>
            ...
            <div className="someOptionInput2">
              <InputNumber
                ...
                max={1000}
                defaultValue={1}
                onChange={(event) => setSizeMin(event)}
              />
            </div>
            <div className="someOptionInput2">
              <InputNumber
                ...
                max={1000}
                defaultValue={999}
                onChange={(event) => setSizeMax(event)}
              />
            </div>
          </div>
          ...
          <div className="slideBar">
            <h5>BUDGET :</h5>
            <IntegerStepMin
              onchange={(event) => setInputValueMin(event)}
              value={inputValueMin}
            />
            <IntegerStepMax
              onchange={(event) => setInputValueMax(event)}
              value={inputValueMax}
            />
          </div>
        </form>
        ...
        annoucements.map((annoucement) => (
          <div key={annoucement.id}>
            <div className="container">
              ...
              <Link to={`/showAnnoucement/${annoucement.id}`}>
